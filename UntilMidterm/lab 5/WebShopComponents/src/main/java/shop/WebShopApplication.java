...
public class WebShopApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		URI response = restTemplate.postForLocation(baseUrl + "/customer", customerDto);

		// get customer
		//todo: call the customer component to get the customer with id 101 and print the result
		CustomerDTO customerResponse = restTemplate.getForObject(baseUrl + "/customer/101", CustomerDTO.class);
		System.out.println(customerResponse.toString());

		//create products

		//todo: call the product component to create the first product
		restTemplate.postForLocation(baseUrl + "/product/Prod1/FirstProduct/12", ProductDTO.class);
		//todo: call the product component to create the second product
		restTemplate.postForLocation(baseUrl + "/product/Prod2/SecondProduct/2.5678", ProductDTO.class);

		//set stock
		//todo: call the product component to set the stock for the first product
		restTemplate.postForLocation(baseUrl + "/product/stock/Prod1/4/A179", ProductDTO.class);

		//get a product
		//todo: call the product component to get the the first product and print the result
		ResponseEntity<String> product1
				= restTemplate.getForEntity(baseUrl + "/product/Prod1", String.class);
		...
		restTemplate.postForLocation(baseUrl + "/cart/Cart1/Prod1/2", null);
		//todo: call the shopping component to add the second product to the cart
		restTemplate.postForLocation(baseUrl + "/cart/Cart1/Prod2/5", null);

		//get the shoppingcart
		//todo: call the shopping component to get the cart and print the result
		restTemplate.getForObject(baseUrl + "/cart/Cart1", ShoppingCartDTO.class).print();

		//checkout the cart
		//todo: call the shopping component to checkout the cart
		restTemplate.postForLocation(baseUrl + "/cart/checkout/Cart1", null);

		//get the order
		//todo: call the order component to get the order and print the result
		restTemplate.getForObject(baseUrl + "/order/Cart1", OrderDTO.class).print();

		//add customer to order
		//todo: call the order component to add a customer to the order
		restTemplate.postForLocation(baseUrl + "/order/setCustomer/Cart1/101", null);

		//confirm the order
		//todo: call the order component to confirm the order
		restTemplate.postForLocation(baseUrl + "/order/Cart1", null);
